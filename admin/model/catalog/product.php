...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_category'])) {
			foreach ($data['product_category'] as $category_id) {
				$paths = $this->db->query("SELECT path_id FROM " . DB_PREFIX . "category_path WHERE category_id = '" . (int)$category_id . "'");
				foreach ($paths->rows as $row) {
					$path_id = (int)$row['path_id'];
					$exists = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id = '" . $path_id . "'");
					if (!$exists->num_rows) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET product_id = '" . (int)$product_id . "', category_id = '" . $path_id . "'");
					}
				}
			}
		}
	...
	public function getProducts($data = array()) {
		...
		if (isset($data['filter_cost']) && trim($data['filter_cost']) !== '') {
			...
			if (preg_match('/^(<=|>=|!=|<|>)[\s]*(-?\d+(?:\.\d+)?)$/', $raw, $m)) {
				$op = $m[1];
				$val = (float)$m[2];
				$sql .= " AND p.cost $op " . $val;
			} else {
				$sql .= " AND p.cost = " . (float)$raw;
			}
		}

		if (isset($data['filter_profit']) && trim($data['filter_profit']) !== '') {
            $raw = html_entity_decode($data['filter_profit'], ENT_QUOTES, 'UTF-8');
            $raw = trim($raw);

            if (preg_match('/^\s*(-?\d+(?:\.\d+)?)\s*(?:-|\.{2})\s*(-?\d+(?:\.\d+)?)\s*$/', $raw, $m)) {
                $low = (float)$m[1]; $high = (float)$m[2];
                if ($low > $high) { $tmp = $low; $low = $high; $high = $tmp; }
                $sql .= " AND (p.price - p.cost) BETWEEN " . $low . " AND " . $high;
            }else if (preg_match('/^\s*(<=|>=|<|>|=|!=)\s*(-?\d+(?:\.\d+)?)\s*$/', $raw, $m)) {
                $op = $m[1]; $val = (float)$m[2];
                $sql .= " AND (p.price - p.cost) " . $op . " " . $val;
                }else if (preg_match('/^\s*(-?\d+(?:\.\d+)?)\s*$/', $raw, $m)) {
                    $val = (float)$m[1];
                    $sql .= " AND (p.price - p.cost) = " . $val;
                }
        }
		...
		if (isset($data['sort']) && $data['sort'] == 'profit') {
    ...
    public function getCategoryFullPath($category_id) {
        ...
        $query = $this->db->query("
            SELECT GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR ' > ') AS full_path
            FROM " . DB_PREFIX . "category_path cp
            LEFT JOIN " . DB_PREFIX . "category_description cd1
            ON (cp.path_id = cd1.category_id AND cd1.language_id = '" . $language_id . "')
            WHERE cp.category_id = '" . (int)$category_id . "'
            GROUP BY cp.category_id
        ");
        return $query->num_rows ? $query->row['full_path'] : '';
    }

	public function getProductCategoryFullPaths($product_id) {
    $language_id = (int)$this->config->get('config_language_id');

    $sql = "
        SELECT GROUP_CONCAT(t.path SEPARATOR ' | ') AS full_path
        FROM (
    ...
    $query = $this->db->query($sql);
    return (!empty($query->row['full_path'])) ? $query->row['full_path'] : '-';
}
