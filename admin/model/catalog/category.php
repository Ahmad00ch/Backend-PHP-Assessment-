...
class ModelCatalogCategory extends Model {
	...
	public function getCategories($data) {
		$sql = "SELECT cp.category_id AS category_id, GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR ' &gt; ') AS name, c.parent_id, c.sort_order, (SELECT GROUP_CONCAT(cd3.name ORDER BY cp2.level SEPARATOR ' &gt; ') FROM " . DB_PREFIX . "category_path cp2 LEFT JOIN " . DB_PREFIX . "category_description cd3 ON (cp2.path_id = cd3.category_id) WHERE cp2.category_id = cp.category_id AND cp2.category_id != cp2.path_id AND cd3.language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY cp2.category_id) AS parent_path FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "category c ON (cp.path_id = c.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd1 ON (c.category_id = cd1.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd2 ON (cp.category_id = cd2.category_id) WHERE cd1.language_id = '" . (int)$this->config->get('config_language_id') . "' AND cd2.language_id = '" . (int)$this->config->get('config_language_id') . "'";
